...
@mcp.resource("dropbox://account")
def get_dropbox_account() -> str:
    """Get Dropbox account resource"""
    try:
        dbx = get_dropbox_client()
        account = dbx.users_get_current_account()
        return f"Dropbox Account: {account.name.display_name} ({account.email})"
    except Exception as e:
        return f"Error accessing Dropbox account: {str(e)}"

@mcp.resource("dropbox://config")
def get_dropbox_config() -> str:
    """Get Dropbox server configuration"""
    return """Dropbox MCP Server Configuration:
